    ...
    try {
      ...
      try {
        const response = await fetch("/api/gemini", {
          ...
          body: JSON.stringify({
            endpoint: endpoint.startsWith("/v1/")
              ? endpoint
              : `/v1/${endpoint}`,
          }),
        ...
        if (!response.ok) {
          ...
          logger.error(
            `Gemini API error: ${errorMessage} (status: ${response.status}, endpoint: ${endpoint}, timestamp: ${new Date().toISOString()})`,
          );
        ...
        if (data.error) {
          ...
          logger.error(
            `Gemini API returned error: ${errorMessage} (endpoint: ${endpoint}, timestamp: ${new Date().toISOString()})`,
          );
          throw new Error(`Gemini API error: ${errorMessage}`);
        }
        ...
        if (error instanceof Error && error.name === "AbortError") {
          logger.error(
            `Gemini API request timeout (endpoint: ${endpoint}, timestamp: ${new Date().toISOString()})`,
          );
      ...
      logger.error(
        `Gemini request failed: ${error instanceof Error ? error.message : String(error)} (attempt: ${retries}/${maxRetries}, endpoint: ${endpoint}, timestamp: ${new Date().toISOString()})`,
      );
