...
const JUPITER_PRICE_CACHE = new Map<
  string,
  { quote: PriceQuote; timestamp: number }
>();
...
export async function getJupiterQuote(
  ...
  try {
    ...
    if (params.asLegacyTransaction)
      queryParams.append("asLegacyTransaction", "true");
    ...
    if (params.maxAutoSlippageBps)
      queryParams.append(
        "maxAutoSlippageBps",
        params.maxAutoSlippageBps.toString(),
      );
    ...
    const response = await fetch(
      `${JUPITER_API_BASE_URL}/quote?${queryParams.toString()}`,
      {
        method: "GET",
        headers: {
