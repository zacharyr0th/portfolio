...
const aptosHandlerInstance = new BaseChainHandler({
  ...
  fetchBalancesImpl: async (publicKey: string, accountId: string) => {
    ...
    try {
      ...
      const validBalances = response.balances
        .filter((balance) => {
          try {
            // Skip spam tokens
            if (
              SPAM_PATTERNS.some((pattern) => pattern.test(balance.token.name))
            ) {
            ...
            const formattedBalance =
              rawBalance / Math.pow(10, balance.token.decimals);

            // Adjust minimum balance based on token
            let minBalance = 0.000001; // Default minimum
            if (
              balance.token.symbol === "APT" ||
              balance.token.symbol === "STAPT"
            ) {
