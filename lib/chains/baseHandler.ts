...
export class BaseChainHandler implements ChainHandler {
  ...
  private canMakeRequest(): boolean {
    ...
    while (
      ...
      (this.requestTimestamps[0] || 0) <
        now - HANDLER_CONSTANTS.RATE_LIMIT_WINDOW
    ) {
      this.requestTimestamps.shift();
    }
    return (
      this.requestTimestamps.length < HANDLER_CONSTANTS.MAX_REQUESTS_PER_WINDOW
    );
  }
  ...
  async fetchBalances(
    ...
    logger.debug(
      `Fetching balances for ${this.config.chainName} wallet: ${publicKey}`,
    );
    ...
    try {
      const request = this.withRetry(async () => {
        const result = await this.config.fetchBalancesImpl(
          publicKey,
          accountId,
        );
        if (!result?.balances) {
