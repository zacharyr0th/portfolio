...
class Logger {
  ...
  private readonly consoleMethodMap: Record<
    LogLevel,
    "debug" | "info" | "warn" | "error"
  >;
  private readonly sensitivePatterns: RegExp[];
  private readonly messageCache: Map<
    string,
  ...
  private constructor() {
    ...
    this.sensitivePatterns = SENSITIVE_PATTERNS.map(
  ...
  private filterSensitiveData(data: unknown): unknown {
    ...
    if (typeof data === "object" && data !== null) {
      ...
      for (const [key, value] of Object.entries(data)) {
        filtered[key] = this.sensitivePatterns.some((pattern) =>
          pattern.test(key),
        )
          ? "[REDACTED]"
