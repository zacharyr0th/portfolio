...
function cleanupStorage(): void {
  try {
    ...
    logger.warn("Error cleaning up localStorage:", {
...
export function useLocalStorage<T>(
  ...
  const readValue = useCallback((): T => {
    ...
    try {
      ...
      logger.warn(`Error reading localStorage key "${safeKey}":`, {
        error: error instanceof Error ? error.message : String(error),
      });
      // Clear invalid data
      localStorage.removeItem(safeKey);
  ...
  const setValue = useCallback(
    (value: T | ((prev: T) => T)) => {
      try {
        ...
        logger.warn(`Error setting localStorage key "${safeKey}":`, {
          error: error instanceof Error ? error.message : String(error),
        });
      }
    },
    ...
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === safeKey && e.newValue !== null) {
        try {
          ...
          logger.warn(`Error handling storage change for "${safeKey}":`, {
            error: error instanceof Error ? error.message : String(error),
          });
        }
      }
