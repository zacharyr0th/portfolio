...
function isRateLimited(): boolean {
  ...
  while (
    requestLog.length > 0 &&
    requestLog[0] !== undefined &&
    requestLog[0] < now - RATE_LIMIT.WINDOW_MS
  ) {
    requestLog.shift();
  }
...
export async function POST(request: Request) {
  ...
  try {
      ...
      if (!endpoint.includes("account/list")) {
        ...
        try {
          ...
          if (!accountListResponse.ok) {
            ...
            logger.error(
              `Failed to get account list: ${errorText} (status: ${accountListResponse.status}, requestId: ${requestId})`,
            );
          ...
          if (error instanceof Error && error.name === "AbortError") {
            logger.error(
              `Account list request timeout (requestId: ${requestId})`,
            );
    ...
    const { signature, encodedPayload } = getMessageSignature(
      endpoint,
      extraParams,
    );

    logger.debug(
      `Fetching from Gemini API: ${endpoint} (requestId: ${requestId})`,
    );
    ...
    try {
      ...
      if (!response.ok || data.error) {
        ...
        logger.error(
          `Gemini API error: ${errorMessage} (status: ${response.status}, requestId: ${requestId}, endpoint: ${endpoint})`,
        );
      ...
      if (error instanceof Error && error.name === "AbortError") {
        logger.error(
          `Gemini API request timeout (requestId: ${requestId}, endpoint: ${endpoint})`,
        );
        return NextResponse.json(
    ...
    logger.error(
      `Error proxying Gemini request: ${errorMessage} (requestId: ${requestId}, endpoint: ${body?.endpoint})`,
    );
