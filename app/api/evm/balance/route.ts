...
export async function GET(request: Request) {
  try {
    ...
    const chainConfig =
      EVM_CHAINS[chain.toLowerCase() as keyof typeof EVM_CHAINS];
    if (!chainConfig) {
    ...
    const [nativeRes, fungibleRes] = await Promise.all([
      ...
      fetch(
        `${SIMPLEHASH_BASE_URL}/fungibles/balances?chains=${simpleHashChain}&wallet_addresses=${address}&include_fungible_details=1&include_prices=1&limit=50`,
        {
          headers,
          next: { revalidate: 60 },
        },
    ...
    if (fungibleData.items) {
      for (const item of fungibleData.items) {
        if (
          item.total_quantity_string &&
          item.total_quantity_string !== "0" &&
          item.fungible_details
        ) {
          const decimals = item.fungible_details.decimals || 18;
          const uiAmount = Number(
            formatUnits(item.total_quantity_string, decimals),
          );
