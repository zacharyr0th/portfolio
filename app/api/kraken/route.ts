...
import {
  ...
  handleApiError,
  type ErrorDetails,
} from "@/lib/utils/core/error-handling";
...
function validateApiKeys() {
  ...
  if (
    process.env.KRAKEN_API_KEY.length < 1 ||
    process.env.KRAKEN_API_PRIVATE_KEY.length < 1
  ) {
...
export async function POST(request: Request) {
  try {
    ...
    while (retries < maxRetries) {
      try {
        ...
        const response = await fetch(
          `${API_URL}/${API_VERSION}/private/${endpoint}`,
          {
            method: "POST",
            headers: {
              "API-Key": apiKey,
        ...
        if (responseData.error && responseData.error.length > 0) {
          ...
          if (
            ...
            if (retries < maxRetries - 1) {
              logger.warn(
                `Retrying Kraken ${endpoint} request due to error: ${errorMessage}`,
              );
              retries++;
    ...
    logger.error("Error in Kraken API route:", appError, {
