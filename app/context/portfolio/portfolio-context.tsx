...
import { AccountService } from "@/app/components/accounts";
...
export function PortfolioProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const initializeAsyncAccounts = async () => {
      try {
        ...
        const baseAccounts = state.accounts.filter(
          (acc) => acc.type !== "cex" && acc.type !== "wallet",
        );
        ...
        const err =
          error instanceof Error
            ? error
            : createError("Failed to load accounts");
  ...
  const handleAccountValueUpdate = useCallback(
      ...
      try {
        ...
        const error =
          err instanceof Error
            ? err
            : createError("Error updating account value");
  ...
  const refreshData = useCallback(async () => {
    try {
      ...
      const nonWalletAccounts = state.accounts.filter(
        (acc) => acc.type !== "wallet",
      );
      dispatch({
        type: "SET_DATA",
      ...
      const error =
        err instanceof Error ? err : createError("Failed to refresh data");
