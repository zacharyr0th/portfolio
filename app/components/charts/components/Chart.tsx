        ...
        data.map((entry) => {
          ...
          return (
            <Cell
              ...
              opacity={
                activeIndex === null || data.indexOf(entry) === activeIndex
                  ? 1
                  : 0.3
              }
            />
...
// Legend component
const Legend = memo(
  ({ data, className, activeIndex, onHover, isPrivate }: LegendProps) => (
    <div
      className={cn(
        "flex flex-col w-full gap-1 p-1 rounded-xl bg-inherit max-w-[250px]",
        className,
      )}
    >
      {data.map((item, index) => (
