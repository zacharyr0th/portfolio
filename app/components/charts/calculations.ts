...
const STAPT_DECIMALS =
  (STAPT_TOKEN_TYPE && TOKEN_SYMBOL_MAP[STAPT_TOKEN_TYPE]?.decimals) || 0;
...
const formatData = (
  data: Record<
    string,
    { value: number; historicalValues?: { timestamp: number; value: number }[] }
  >,
...
> => {
  return calculateDistribution(accounts, (account) => {
    ...
    return (
      PLATFORM_MAP[account.platform?.toLowerCase() ?? ""] || account.platform
    );
  });
};
...
> => {
  ...
  accounts.forEach((account) => {
    ...
    if (
      (account.type === "cex" || account.type === "wallet") &&
      ...
      account.resources.forEach((resource) => {
        ...
        if (!coinData || typeof coinData !== "object" || !("value" in coinData))
          return;
        ...
        if (Array.isArray(historicalValues) && historicalValues.length > 0) {
          const validHistoricalValues = historicalValues.filter(
            (v) =>
              v &&
              typeof v === "object" &&
              "value" in v &&
