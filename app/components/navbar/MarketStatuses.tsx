...
export const MarketStatuses = memo(
  ({ activeMarket, onMarketClick }: MarketStatusesProps) => {
      ...
      () =>
        MARKET_CONFIG.map((market, index) => (
          <React.Fragment key={market.timeZone}>
            ...
            <div
              className={cn("flex items-center", isMobile && "min-w-[52px]")}
            >
              <MarketStatus
                timeZone={market.timeZone}
                labelPrefix={market.labelPrefix}
              />
            </div>
          </React.Fragment>
        )),
    ...
    return (
      <div
        className={cn(
          "flex items-center",
          isMobile
            ? "gap-2 w-max px-1 py-0.5 bg-muted/20 rounded-lg"
            : "gap-2 sm:gap-3",
        )}
      >
        {marketItems}
