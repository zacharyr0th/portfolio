...
function useAccountFilters(initialShowArchived: boolean = false) {
  ...
  return {
    ...
    setHideCards: useCallback(
      (value: boolean) => setFilter("hideCards", value),
      [setFilter],
    ),
...
export function AccountList({
  ...
  const { accounts: portfolioAccounts, handleAccountValueUpdate } =
    usePortfolio();

  // Memoize the filter function
  const filterAccount = useCallback(
    (account: Account, filters: FilterState) => {
      // Archive filter
      if (account.type === "wallet") {
        const walletAccount = account as WalletAccount;
        if (!filters.showArchived && walletAccount.status === "inactive")
          return false;
      }
  ...
  const filteredAccounts = useMemo(() => {
    // Apply filters
    const filtered = typeFilteredAccounts.filter((account) =>
      filterAccount(account, filters),
    );
    // Sort accounts
