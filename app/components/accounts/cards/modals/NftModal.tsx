...
export function NftModal({ isOpen, onClose, walletAddress, chain }: NftModalProps) {
    ...
    return (
        <div className="bg-card rounded-xl overflow-hidden border border-border hover:border-border/80 transition-all hover:shadow-md">
            ...
            <div className="p-4 space-y-2">
                <h3 className="font-medium truncate text-base" title={nft.name || 'Unnamed NFT'}>
                    {nft.name || 'Unnamed NFT'}
                </h3>
                <p className="text-sm text-muted-foreground truncate" title={nft.collection.name}>
                    {nft.collection.name}
                </p>
                {nft.floor_price && (
