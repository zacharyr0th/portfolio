import {
  accountConfig,
  getWalletConfigs,
} from "@/app/components/accounts/config";
import type {
  WalletAccount,
  BankAccount,
  BrokerAccount,
...
export class AccountService {
  ...
  private static addDefaultFields<T extends Partial<Account>>(
    account: T,
  ): T & { value: number; lastUpdated: string } {
    return {
      ...account,
      value: 0,
  ...
  private static async fetchWithRetry<T>(
    ...
    try {
      ...
      if (this.retryCount < this.MAX_RETRIES) {
        ...
        logger.warn(
          `Retrying fetch for ${key} in ${delay}ms (attempt ${this.retryCount})`,
        );
  ...
  static async getWalletAccounts(): Promise<WalletAccount[]> {
    return this.withCache("wallets", async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to fetch wallet configurations: ${response.statusText}`,
          );
        }
        ...
        return SUPPORTED_CHAINS.flatMap((chain) =>
          Object.values(configs[chain] || {}).map((wallet) =>
            this.addDefaultFields(wallet as WalletAccount),
          ),
        );
      } catch (error) {
        logger.error(
          "Failed to load wallet accounts:",
  ...
  static getBankAccounts(): BankAccount[] {
    return Object.values(accountConfig.banks).map((bank) =>
      this.addDefaultFields(bank as BankAccount),
    );
  }

  static getBrokerAccounts(): BrokerAccount[] {
    return Object.values(accountConfig.brokers).map((broker) =>
      this.addDefaultFields(broker as BrokerAccount),
    );
  }
  ...
  static async updateCexAccountValue(account: CexAccount): Promise<number> {
    try {
      if (account.platform === "Gemini") {
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to fetch Gemini balance: ${response.statusText}`,
          );
        }
      ...
      logger.error(
        `Failed to update ${account.platform} balance:`,
        error instanceof Error ? error : new Error(String(error)),
      );
      return account.value;
    }
  ...
  static async getCexAccounts(): Promise<CexAccount[]> {
    return this.withCache("cex-accounts", async () => {
      const accounts = Object.values(accountConfig.cex).map((cex) =>
        this.addDefaultFields(cex as CexAccount),
      );
  ...
  static getCreditAccounts(): Account[] {
    return Object.values(accountConfig.credit || {}).map((credit) =>
      this.addDefaultFields(credit as Account),
    );
  }
  ...
  static async getActiveWalletsByChain(
    chain: WalletAccount["chain"],
  ): Promise<WalletAccount[]> {
    const wallets = await this.getWalletAccounts();
    return wallets.filter(
      (wallet) => wallet.chain === chain && wallet.status === "active",
    );
  }
